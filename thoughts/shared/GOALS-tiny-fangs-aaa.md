...
**Last updated:** 2026-07-30 00:30 EDT  
...
- **USER DIRECTION (2026-07-30): materials are placeholder — the real path is IMAGE COMPOSITING.** "We don't really need materials. We'll be using all images and just merging them together or moving them around." The procedural CSS material system (parchment gradients, chassis skin) stays only as a placeholder; do NOT invest further in procedural material work. What survives and matters: chassis geometry, §7.4 safe rects, homography quad mapping, anchor layout, and the FLIP/positioning machinery — that IS the compositing layer user images plug into. Also: **multiplayer is not a priority** — single-player focus until visuals are done; keep MP from breaking, don't build on it.
- **ACCOMPLISHED (2026-07-30): Task 68 — wave-2 integration (4 agents) + a real code-split regression caught and fixed.** Landed: docs refresh (MEMORY/CHANGELOG/ARCHITECTURE/CLAUDE/README), RSP-02 shell-selection REPAIR (new src/viewport.js single source of truth at the CSS 900px threshold; mp-client was using 600 so 601-899 revealed BOTH shells; frozen characterization spec converted to a correctness contract that also asserts the stylesheet's shell equals the revealed shell), Phase 13 quality tiers wired into the shipping shell with a persisted user override chip (desktop-high byte-identical; desktop-low drops antialias/particles/light-spill; static reuses the RSP-07 downgrade), and MP-07 proven structurally unreachable with an additive protocol test. **REGRESSION FOUND AND FIXED: the lazy code-split made aaa-shell.css lazy too, so in aaa mode the setup screen, modals, and coin had NO styling until a game mounted** — CSS now loads eagerly (19 kB) while the heavy three.js chunk stays lazy. Verified on a quiet tree: units 630/630, server-process 18/18, multiplayer 19/19, visual 27/27, e2e 110/116 with 3 adaptive-trajectory failures (see BLOCKED below) and both modes booting a live single-player game with zero page errors.
- **KNOWN-FRAGILE (2026-07-30): three adaptive-trajectory E2E tests** (aaa-actions "retreat swaps", aaa-motion "retreat FLIPs", aaa-selectors "target selection") fail because the rival AI now wins during their adaptive board-building loops — the seeded trajectory shifted under wave-2 timing. These assert PRODUCT behavior already proven elsewhere (retreat via aaa-affordances ACT-11, targeting via the selector's own ownership/highlight assertions); the fragility is in the tests' turn-advancement strategy, not the shell. FIX APPROACH (do not weaken assertions): give these tests a deterministic board via the fixture route instead of playing the AI to reach a shape.
- **ACCOMPLISHED (2026-07-29): Task 67 — parallel gap closure (4 subagents): 13 matrix rows closed, matrix now 91 covered / 11 missing / 1 manual (from 78/24/1).** New specs: aaa-status (STA-01..04, STA-08, OVR-10 — poison via REAL Hexweaver with exact 10-tick heal-netted accounting), aaa-affordances + classic-hold-guards (ACT-07B/09/11/15, OVR-02, INP-10 — actionLock proven to block a synchronous duplicate before the engine), multiplayer deck-parity (SET-03, 5/5). THREE REAL DEFECTS FIXED: bench status charms never rendered (renderStatuses wired only to actives), production card art 404'd (hard-coded /src/ path → import.meta.glob, dist now emits hashed webp), plus task-66's dead-screen fallback. Phase 13: AAA shell code-split behind a lazy import (classic payload 739.85→170.90 kB, gzip 198→49.6), DPR capped, idle 8.33ms — OPEN: quality tiers are dead code on the shipping path and no user override exists. Contract clarifications recorded (Set affordability is per-option, reveal timings 2500/5000/1000ms). Evidence: visual 27/27, E2E 115/115, MP deck-parity 5/5, units 612/612, build clean. Handoff: task-67-parallel-gap-closure.md. NEXT: RSP-02 shell fix, quality tiers + override, MP parity block, Phase 14 critics.
- **ACCOMPLISHED (2026-07-29): Task 66 — behavior matrix reconciled (78 covered / 24 missing / 1 manual, from 70/32/1) + RSP-07 dead-screen defect FIXED.** Eight rows flipped with evidence pointers; ranked gap list recorded in the matrix. The WebGL fallback did not actually exist: aaa CSS hides classic shells and a failed mount left a dead screen — renderAaaShell now downgrades the flag to classic; aaa-fallback E2E blocks WebGL and plays a real classic turn. Full E2E 99/99; visual 27/27; units 612/612. Handoff: task-66-matrix-reconciliation.md. NEXT (user-directed parallel agents): status-playback quartet, affordance tables, MP parity sweep, Phase 13 budgets.
